...
router.get("/thread", ensureAuthenticated, async (req, res) => {
  try {
    const threads = await Thread.find({ author: req.user.id }).sort({ updatedAt: -1 });
...
router.get("/thread/:threadId", ensureAuthenticated, async (req, res) => {
  try {
    const thread = await Thread.findOne({ threadId: req.params.threadId, author: req.user.id });
...
router.delete("/thread/:threadId", ensureAuthenticated, async (req, res) => {
  try {
    const deleted = await Thread.findOneAndDelete({ threadId: req.params.threadId, author: req.user.id });
...
router.post("/chat", ensureAuthenticated, async (req, res) => {
  const { threadId, message, imageBase64, imageMimeType } = req.body;
  if (!threadId || !message) return res.status(400).json({ error: "Missing required fields" });

  try {
    let thread = await Thread.findOne({ threadId, author: req.user.id });
    if (!thread) {
      thread = new Thread({
        threadId,
    ...
    const assistantReply = await getGeminiResponse(thread.messages, imageBase64 || null, imageMimeType || null);
    if (!assistantReply) return res.status(502).json({ error: "AI did not return a valid reply" });
